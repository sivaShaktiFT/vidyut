...
describe("Panchanga primitives", () => {
  ...
  it("round-trips a local civil clock through Julian Day", () => {
    ...
    expect([date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes()]).toEqual([2026, 8, 11, 1, 0]);
  });
  it("applies source-chart varga conventions", () => {
  ...
  it("ports the source chart's pure dasha and aspect rules", () => {
  ...
  it("resolves an IANA civil time without a fixed offset", () => {
    expect(zonedCivilDateToJulianDay({ year: 2026, month: 8, day: 11 }, "Asia/Kolkata", 6.5)).toBeCloseTo(civilDateToJulianDay({ year: 2026, month: 8, day: 11 }, 5.5, 6.5), 8);
  });
  ...
  it("ships the permitted source observance catalogue", () => {
    ...
    expect(SOURCE_OBSERVANCES.find((item) => item.name === "Dīpāvalī")?.desc).toBe("Festival of Lights");
  ...
  it("matches the recorded source-compatible Bengaluru fixture", async () => {
    ...
    expect([panchanga.tithi.name, panchanga.nakshatra.name, panchanga.yoga.name, panchanga.karana.name]).toEqual(["Amavasya", "Punarvasu", "Siddhi", "Vishti"]);
    expect(panchanga.observances.map((item) => item.name)).toEqual(["Māsika Śivarātri", "Maṅgalavāra Vrata"]);
    ...
    const chart = calculator.birthChart({ year: 1990, month: 1, day: 1, hour: 12, second: 17 }, location);
